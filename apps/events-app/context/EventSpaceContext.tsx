...
interface EventSpaceContextProps {
  ...
  setEventSpace: React.Dispatch<React.SetStateAction<EventSpaceDetailsType | null>>;
}
type EvenSpaceProps = {
  children: ReactNode;
};
...
export const EventSpaceProvider = ({ children }: EvenSpaceProps) => {
  const [eventSpace, setEventSpace] = useState<EventSpaceDetailsType | null>(null);

  return <EventSpaceContext.Provider value={{ eventSpace, setEventSpace }}>{children}</EventSpaceContext.Provider>;
};
