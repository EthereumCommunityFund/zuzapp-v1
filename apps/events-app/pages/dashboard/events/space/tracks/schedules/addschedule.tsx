...
import { EventSpaceDetailsType, InputFieldType, LocationUpdateRequestBody } from '@/types';
...
export default function AddSchedulePage() {
  ...
  const handleRemoveSpeaker = (index: number) => {
    const updatedItems = [...speakers.slice(0, index), ...speakers.slice(index + 1)];
  ...
  useEffect(() => {
    const fetchLocationDetails = async () => {
      try {
        ...
        console.log(result, 'locationsfetched');
    ...
    const fetchTags = async () => {
      try {
        ...
        console.log(result, 'tags fetched');
    ...
    const fetchSpeakers = async () => {
      try {
        ...
        console.log(result, 'speakers fetched');
    ...
    <div className="flex items-start gap-[60px] self-stretch px-10 py-5">
      ...
      <div className="flex flex-col items-start gap-[17px] flex-1">
        <div className="flex items-center gap-[17px] self-stretch">
          <Button className="rounded-[40px] py-2.5 px-3.5 bg-bgPrimary border-none hover:bg-[#363636] duration-200 text-textSecondary hover:text-textSecondary" size="lg" leftIcon={HiArrowLeft}>
            Back
          </Button>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10 w-full">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-3">
                          ...
                          <FormControl>
                            <RadioGroup defaultValue={field.value} className="flex flex-col md:flex-row justify-between" {...field}>
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex flex-col items-start gap-5 self-stretch w-full pt-5">
                        <div className="flex flex-col gap-[14px] items-start self-stretch w-full">
                          <h2 className="text-lg font-semibold leading-[1.2] text-white self-stretch">Video Call Link</h2>
                          <InputFieldDark type={InputFieldType.Link} placeholder="Type URL" value={videoLink} onChange={(e) => setVideoLink((e.target as HTMLInputElement).value)} />
                        </div>

                        <div className="flex flex-col gap-[14px] items-start self-stretch w-full">
                          <h2 className="text-lg font-semibold leading-[1.2] text-white self-stretch">Live Stream Link</h2>
                          <InputFieldDark type={InputFieldType.Link} placeholder="Type URL" value={liveLink} onChange={(e) => setLiveLink((e.target as HTMLInputElement).value)} />
                        </div>
                      </div>
                    </div>
                    <line></line>
                            ...
                            <div className="flex flex-col gap-[14px] items-start self-stretch w-full">
                              ...
                              <InputFieldDark
                                ...
                                onChange={(e) =>
                                  setEventItem({
                                    ...eventItem,
                                    speaker_name: (e.target as HTMLInputElement).value,
                                  })
                                }
                            ...
                            {speakers?.map((speaker: any, index: number) => (
                              <div key={index} className="flex gap-2.5 items-center rounded-[8px] px-2 py-1.5 bg-white bg-opacity-10">
                                <button type="button" className="flex gap-2.5 items-center">
                                  ...
                                  <span className="text-lg font-semibold leading-[1.2] text-white self-stretch">{speaker.speaker_name}</span>
                                </button>
                              </div>
                            ))}
                          </div>
