...
export default function EventSpaceDashboard(props: IProps) {
  ...
  const handlePublishEvent = async () => {
    ...
    if (
      ...
      console.error(
        "Event space does not meet the minimum requirements for publishing"
      );
      setDialogContent({
        title: "Error!",
        description:
  ...
  const handleButtonClick = (type: SpaceDashboardCardType) => {
    if (type === SpaceDashboardCardType.PublishEvent) {
      ...
      type === SpaceDashboardCardType.EnterEventDetails ||
      type === SpaceDashboardCardType.EditDetails
    ) {
      router.push({
        pathname: `/dashboard/events/space/details/`,
        query: { event_space_id: event_space_id },
      ...
      <div className="flex flex-col items-center gap-5 flex-1 w-full">
        {
          <div className="xl:w-4/5 w-full md:w-[90%] px-2.5">
            {isFirst === SpaceDashboardType.New.toString() &&
              eventSpace?.status === "draft" && eventSpace.name === '' && (
                <>
                  <Dialog open>
                    <DialogContent className="sm:max-w-[425px] h-1/3 md:max-w-none w-[600px]">
                      <DialogHeader>
                        <DialogTitle className="text-2xl">
                          Welcome to your Event Space
                        </DialogTitle>
                        <DialogDescription className="text-xl font-bold">
                          First, you'll need to enter the details of your main
                          event.
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter className="pt-5 self-end text-2xl">
                        <Button
                          ...
                          onClick={() =>
                            handleButtonClick(
                              SpaceDashboardCardType.EnterEventDetails
                            )
                          }
                        >
                          Enter Event Details
