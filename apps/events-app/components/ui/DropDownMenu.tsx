  ...
  return (
    <>
      <Listbox as={'div'} className={cn('w-full relative', className)} value={selectedCategories} multiple={multiple} onChange={selectedCategories}>
        ...
        >
          <Listbox.Options
            className={cn('absolute right-0 z-10 mt-2 w-full pb-2 bg-componentPrimary origin-top-right rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none', optionsClassName)}
          >
            {data.map((item, idx) => (
