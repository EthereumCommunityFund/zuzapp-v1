  ...
  return (
    <div className="flex flex-col py-5 px-4 items-center gap-8 rounded-2xl border bg-[#2E3131] border-white/10 w-full">
      <div className="flex flex-col items-start gap-[10px] self-stretch">
        ...
        <Button variant={cardType === SpaceDashboardCardType.EditDetails || cardType === SpaceDashboardCardType.OpenSettings ? 'primary' : 'primaryGreen'} className="w-full flex justify-center rounded-3xl text-xl" leftIcon={buttonIcon} onClick={(e) => { onCardClick && onCardClick(cardType) }}>{buttonName}</Button>
      </div>
    </div>
  )
}