...
const EventSpaceDetails: React.FC<EventSpaceDetailsProps> = ({
  ...
  const [bannerMetadata, setBannerMetadata] = useState(null);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const banner_url = await uploadImage(bannerMetadata, "events");
        ...
        <div className="lg:flex hidden flex-col pt-3 rounded-s-xl opacity-70 w-[300px] gap-5 fixed">
          ...
          <div className="flex flex-col gap-3">
            {eventDetailsList.map((eventDetailsItem, index) => (
              <div
                key={index}
                className=" rounded-xl flex flex-col gap-1 pl-3 py-2 hover:cursor-pointer w-[230px] hover:bg-[#292929] duration-200"
                onClick={() => scrollToRef(sectionRefs[index])}
              >
                <div className="text-lg font-semibold opacity-90">
                  {eventDetailsItem.name}
                </div>
                <div className="text-xs font-light opacity-60">
                  {eventDetailsItem.description}
                </div>
              </div>
            ))}
          </div>
                ...
                >
                  <div className="flex flex-col gap-[34px] w-full">
                    ...
                    <EventBanner
                      banner={banner}
                      setBanner={setBanner}
                      setBannerMetadata={setBannerMetadata}
                    />
                    ...
                    >
                      <div className="flex flex-col gap-2.5">
                        ...
                        <span className="opacity-70 h-[18px] font-normal text-[13px] leading-[18.2px] tracking-[0.13px] self-stretch">
                          These will be shared as attributes by subsequent
                          Tracks & Sessions you create.
                        </span>
                      </div>
