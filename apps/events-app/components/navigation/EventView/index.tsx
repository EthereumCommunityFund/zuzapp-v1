...
export default function EventViewNavigation() {
  ...
  return (
    <>
      ...
      <nav
        className={`z-50 lg:w-[250px] w-full mt-[4.3rem] lg:mt-0 sticky top-5 backdrop-blur-3xl lg:fixed flex flex-col lg:h-screen border-r border-r-gray-800 lg:py-10 lg:pl-10 md:text-base text-[12px] transition-transform duration-300 ${
          dashboardOpen && 'open'
        }`}
      >
                ...
                {routes.map(
                  (route, index) =>
                    route.name && (
                      <li
                        key={index}
                        className={`flex flex-nowrap items-center font-medium text-sm cursor-pointer space-x-2 py-2 px-3 lg:rounded-xl md:hover:border-b lg:hover:border-0 duration-200 ${
                          router.pathname.includes(route.path) ? 'lg:bg-white/10 sm:border-b-[2px] sm:border-white/80 lg:border-0' : 'opacity-60 lg:hover:bg-white/20'
                        }`}
                      >
                        {route.icon && <route.icon size={25} />}
                        <Link className="whitespace-nowrap" href={`${route.path}?event_space_id=${event_space_id}`}>
                          {route.name}
                        </Link>
                      </li>
                    )
              ...
              eventSpace?.creator_id === user.id && (
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="blue" size="lg" className="rounded-full block sm:hidden  sm:w-full lg:w-fit justify-center" leftIcon={BiPlusCircle}>
                      Add a Session
                    </Button>
                  </DialogTrigger>
                  {
                    <DialogContent className="md:w-3/5 md:h-3/5 overflow-x-auto sm:w-3/4">
                      <AddScheduleForm isQuickAccess={true} updateIsLoading={updateIsLoading} event_space_id={event_space_id as string} isFromEventView={true} />
                    </DialogContent>
                  }
                </Dialog>
              )
