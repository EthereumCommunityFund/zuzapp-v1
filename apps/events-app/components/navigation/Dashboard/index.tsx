...
export default function DashboardNavigation() {
  ...
  const allowedPaths = ['https://www.zuzagora.com/', 'https://www.guilded.gg/Zuzalu/blog/Commuity-Blog', 'https://www.guilded.gg/Zuzalu/blog/Announcements'];
  ...
  return (
    <>
      ...
      <div className={` hidden fixed inset-0 bg-black/10 z-10 ${dashboardOpen ? 'block' : 'hidden'}`} onClick={handleClick}></div>
      <nav className={`dashboard-menu min-w-[260px] z-10 fixed flex flex-col h-screen border-r border-r-gray-800 bg-[#2F3232] py-10 px-6 transition-transform duration-300 ${dashboardOpen && 'open'}`}>
        <div className="flex-1 flex flex-col">
              ...
              {routes.map((route, index) => (
                <li
                  ...
                  className={`flex items-center text-sm transition duration-200 space-x-2 py-2 font-semibold px-3 hover:bg-white/20 rounded-xl ${
                    router.pathname === route.path ? 'bg-white/20' : 'opacity-60'
                  }`}
                >
                ...
                {routes.map(
                  (route) =>
                    ...
                    route.options.map((option) => (
                      <li key={option.path} className={`flex  ${router.pathname === option.path ? 'bg-white/0' : 'opacity-60'}`}>
                        <div>
            ...
            <ul className="flex mt-5 flex-col gap-[31px]">
              <li className="flex items-center space-x-2">
                <Link href={'/dashboard/events/myspaces'} className="w-full">
                  <Button size="base" variant="primaryGreen" className="rounded-full w-full text-base opacity-70" leftIcon={FaCog}>
                    My Event Spaces
                  </Button>
                </Link>
              </li>
          ...
          <div className="mt-5 py-[15px] px-[13px] gap-3.5 flex flex-col rounded-2xl bg-[#2C2D2D] w-[240px]">
            ...
            <Label className="text-white/70 text-xs font-normal">Take part in building Zuzalu tools and discussions!</Label>

            <Label className="text-white text-l border_top">Build With Us</Label>
            <div className="menu_cta" onClick={() => window.open('https://www.guilded.gg/r/zzME2L6mXR?i=dVbg9yzd', '_blank')}>
              <img src="/images/guilded_logo.svg" />
              <span>Join Guilded</span>
            </div>

            <Label className="text-white text-l">Participate in Discussions:</Label>
            <div className="menu_cta" onClick={() => window.open('https://www.zuzagora.com/', '_blank')}>
