...
export default function DashboardNavigation() {
  ...
  return (
    <>
      ...
      <nav className={`dashboard-menu min-w-[260px] fixed flex flex-col h-screen border-r border-r-gray-800 bg-[#2F3232] py-10 px-6 transition-transform duration-300 ${dashboardOpen && 'open'}`}>
        <div className="flex-1 flex flex-col opacity-70">
          <div className="mt-10 flex-1">
            <ul className="space-y-2">
              {routes.map((route, index) => (
                <li key={route.path} className={`flex items-center space-x-2 py-1 px-3 hover:bg-white/20 rounded-3xl ${router.pathname === route.path && 'bg-white/20'}`}>
                  <route.icon size={30} />
            ...
            <ul className="flex flex-col gap-[31px]">
              <li className="flex items-center space-x-2">
                <Link href={'/dashboard/events/myspaces'} className="w-full">
                  <Button size='base' variant={'primary'} className="rounded-full w-full text-base" leftIcon={FaCog}>
                    My Event Spaces
                  </Button>
                </Link>
              </li>
